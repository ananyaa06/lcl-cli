...
extension LCLCLI {
  struct SpeedTestCommand: AsyncParsableCommand {
    ...
    @Option(
      name: .shortAndLong,
      help: "Specify the unit (MBps or mbps) for the speed test results. Default is \"mbps\"")
    var unit: String = "mbps"

    @Option(
      name: .shortAndLong,
      help:
        "Specify the direction of the test. A test can be of three types: download, upload or downloadAndUpload"
    )
    var type: TestType
    ...
    static let configuration = CommandConfiguration(
      commandName: "speedtest", abstract: "Run speedtest using the NDT test infrastructure.")
    ...
    func run() async throws {
      ...
      let downloadSummary = prepareSpeedTestSummary(
        ...
        for: .download, unit: speedTestUnit)
      ...
      case .download:
        generateSpeedTestSummary(
          downloadSummary, kind: .download, formats: outputFormats, unit: speedTestUnit)
      case .upload:
        generateSpeedTestSummary(
          uploadSummary, kind: .upload, formats: outputFormats, unit: speedTestUnit)
