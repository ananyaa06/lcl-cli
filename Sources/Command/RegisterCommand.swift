...
extension LCLCLI {
  struct RegisterCommand: AsyncParsableCommand {
    ...
    @Option(
      name: .shortAndLong, help: "Path to the SCN credential file given by the SCN administrator.")
    var filePath: String
    ...
    func run() async throws {
      ...
      let registration = RegistrationModel(
        sigmaR: sigma_r.hex, h: h_concat.hex, R: validationResult.R.hex)
      ...
      switch await NetworkingAPI.send(
        to: NetworkingAPI.Endpoint.register.url, using: registrationJson)
      {
      case .success:
