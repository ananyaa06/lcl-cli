...
extension LCLCLI {
  struct MeasureCommand: AsyncParsableCommand {
    ...
    func run() async throws {
      ...
      guard
        try ECDSA.verify(
          message: validationResultJSON, signature: sigData, publicKey: ecPrivateKey.publicKey)
      else {
        throw CLIError.contentCorrupted
      }

      let pingConfig = try ICMPPingClient.Configuration(
        endpoint: .ipv4("google.com", 0), deviceName: deviceName)
      ...
      if configuration == nil {
        var picker = Picker<CellularSite>(
          title: "Choose the cellular site you are currently at.", options: sites)
      ...
      let downloadMeasurement = computeLatencyAndRetransmission(
        speedTestResults.downloadTCPMeasurement, for: .download)
      let uploadMeasurement = computeLatencyAndRetransmission(
        speedTestResults.uploadTCPMeasurement, for: .upload)

      let downloadSummary = prepareSpeedTestSummary(
        data: speedTestResults.downloadSpeed, tcpInfos: speedTestResults.downloadTCPMeasurement,
        for: .download, unit: .Mbps)
      let uploadSummary = prepareSpeedTestSummary(
        data: speedTestResults.uploadSpeed, tcpInfos: speedTestResults.uploadTCPMeasurement,
        for: .upload, unit: .Mbps)

      generatePingSummary(summary, for: .icmp, formats: outputFormats)
      generateSpeedTestSummary(
        downloadSummary, kind: .download, formats: outputFormats, unit: .Mbps)
      ...
      do {
        ...
        let sig_m = try ECDSA.sign(
          message: serialized, privateKey: ECDSA.deserializePrivateKey(raw: skData))
        let measurementReport = MeasurementReportModel(
          sigmaM: sig_m.hex, hPKR: hpkrData.hex, M: serialized.hex, showData: showData)
        ...
        let result = await NetworkingAPI.send(
          to: NetworkingAPI.Endpoint.report.url, using: reportToSent)
        switch result {
        case .success:
