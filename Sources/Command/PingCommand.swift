...
extension LCLCLI {
    struct ICMPCommand: ParsableCommand {
        ...
        @Option(
            name: .shortAndLong,
            help:
                "Specify the number of times LCLPing runs the test. The number has to be greater than 0. Default is 10."
        )
        var count: UInt64?
        ...
        @Option(
            name: .shortAndLong,
            help:
                "The wait time, in milliseconds, between sending consecutive packet. Default is 1000 ms."
        )
        var interval: UInt64?
        ...
        @Option(
            name: .long,
            help:
                "Time-to-live for outgoing packets. The number has to be greater than 0. Default is 64. This option applies to ICMP only."
        )
        var ttl: UInt8?

        @Option(
            name: .long,
            help: "Time, in milliseconds, to wait for a reply for each packet sent. Default is 1000 ms."
        )
        var timeout: UInt64?
    ...
    struct HTTPCommand: ParsableCommand {
        ...
        @Option(
            name: .shortAndLong,
            help:
                "Specify the number of times the ping test runs. The number has to be greater than 0. Default is 10."
        )
        var count: UInt64?
        ...
        @Option(
            name: .long,
            help:
                "Time, in milliseconds, that the HTTP client will wait when connecting to the host. Default is 5000 ms."
        )
        ...
        @Option(
            name: .long,
            help:
                "Time, in milliseconds, HTTP will wait for the response from the host. Default is 1000 ms."
        )
        var readTimeout: UInt64?

        @Option(
            name: .long,
            help: "HTTP Headers to be included in the request. Use comma to separate <Key>:<Value>."
        )
        var headers: String?
        ...
        @Flag(
            name: .long,
            help:
                "Use Server-Timing in HTTP header to calculate latency. Server should support Server-Timing in HTTP response. Otherwise, a default 15ms will be taken into consideration."
        )
        var useServerTiming: Bool = false

        @Flag(
            help:
                "Use URLSession on Apple platform for underlying networking and measurement. This flag has no effect on Linux platform."
        )
        ...
        func run() throws {
            ...
            if let headers = headers {
                ...
                for element in headers.split(separator: ",") {
